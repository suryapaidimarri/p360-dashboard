...
import { DynamicChart, KPICard, ChartCard, WidgetDot, Toggle, ResizeHandle, NewDashCanvas, ChartThumbSvg, KPI_BG, ALLOY, Widget } from './WidgetComponents'
...
export default function ClientWorkspace({ params }: { params: { id: string } }) {
  ...
  // ── Ctx objects passed to top-level component functions ─────────────────
          ...
          {isEmptyDash ? (
            ...
            <div style={{ padding:16 }}>
              ...
              <div style={{ display:'flex', flexWrap:'wrap' as const, gap:10, marginBottom:10 }}>
                {widgets.map(w => <KPICard key={w.id} w={w} _ctx={_kpiCtx}/>)}
              </div>
                  ...
                  <div onClick={e => { e.stopPropagation(); if (editMode) startEdit(widgets[3]) }}
                    ...
                    {editMode && (
                      <div onClick={e => e.stopPropagation()} style={{ position:'absolute', top:6, right:6, zIndex:10, display:'flex', gap:4 }}>
                        ...
                        <WidgetDot wid="bounce" onEdit={() => startEdit(widgets[3])} _ctx={_wdCtx}/>
                      </div>
                    )}
              ...
              <div style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:10, marginTop:10 }}>
                {dynamicWidgets.map(w => (
                  <div key={w.id}
                    ...
                    {editMode && (
                      <div onClick={e => e.stopPropagation()} style={{ position:'absolute', top:6, right:6, zIndex:10, display:'flex', gap:4 }}>
                        <WidgetDot wid={w.id} onEdit={() => startEdit(w)} _ctx={_wdCtx}/>
                      </div>
                    )}
